...
def  main():
  ...
  installer = "\installer\FftwLearning Installer.exe"
   
  if demo:
    installer = "\installer\FftwLearning Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\FftwLearning manual.pdf" 
  ]
  ...
  ZIPNAME = "FftwLearning-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "FftwLearning-v" + FULLVERSIONSTR + "-win-demo.zip"
